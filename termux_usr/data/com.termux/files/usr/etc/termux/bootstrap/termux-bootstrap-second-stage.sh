...
run_bootstrap_second_stage() {
	...
	output="$(ln -s "termux-bootstrap-second-stage.sh" \
		"/data/data/com.termux/files/usr/etc/termux/bootstrap/termux-bootstrap-second-stage.sh.lock" 2>&1)"
	return_value=$?
...
then delete the '/data/data/com.termux/files/usr/etc/termux/bootstrap/termux-bootstrap-second-stage.sh.lock' \
...
'/data/data/com.termux/files/usr/etc/termux/bootstrap/termux-bootstrap-second-stage.sh.lock'"
...
of the Termux app APK that is not compatible with the device."
			log_error "Uninstall and reinstall the correct APK variant of the Termux app."
...
it must be run as the TERMUX__UID '$TERMUX__UID' exported by the Termux app."
		return 1
	fi
